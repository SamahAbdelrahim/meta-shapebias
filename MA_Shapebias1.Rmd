...
mod_log_age <- rma.mv(d ~ log_mean_age_months, 
                      ...
                      random = ~1 | as.factor(Title) / 
                        as.factor(exp_num), 
                      slab = Title, 
                      data = filter(df_shape_nonendo, !is.na(log_mean_age_months)))
...
## English and Est Asian:
```{r}
df_shape <- df_shape %>% group_by(language) %>%
  mutate(language_grp = case_when(language == "english" ~ "english",    language %in%c("chinese","japanese", "korean") ~  "east_asian",
          language %in% c("spanish", "german") ~ "indo-european", 
          TRUE ~ "other"
  ))
