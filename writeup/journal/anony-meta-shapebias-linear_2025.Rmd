...
mod_ployage_ie <- rma.mv(d ~ mean_age_months_centered36 * indoeuropean, 
                         V = d_var,
                         random = ~ 1 | ID/exp_num/index, 
                         slab = short_cite, 
                         data = df_shape_solid)
...
All models including age fit better than the model without age ($\Delta_{AIC}$ between `r round(AICs$delta_aic[AICs$model == "logage"], 2)` and `r round(AICs$delta_aic[AICs$model == "polyage"], 2)`). The linear model fit best ($\Delta_{AIC} =$ `r round(AICs$delta_aic[AICs$model == "polyage"] - AICs$delta_aic[AICs$model == "age"], 2)`), so we adopted a linear function for our subsequent analysis with a significant overall estimate of `r round(polycoeff$coeff[1], 3)` ($p=$ `r papaja::apa_p(mod_age$pval)[1]`) that decreases with quadratic age `r round(polycoeff$coeff[2], 5)` ($p=$ `r papaja::apa_p(mod_age$pval)[2]`) and a heterogeneity of ($I^2 = `r round(I2_linear, 2)`$).
