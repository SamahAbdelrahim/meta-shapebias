...
shorttitle        : "SHAPE BIAS META-ANALYSIS"

author: 
  - name          : "Samah Abdelrahim"
    affiliation   : "1"
    corresponding : yes
    address       : "Building 420, 450 Jane Stanford Way, Stanford, CA 94305, mailcode: 2130"
...
authornote: |
  \textsuperscript{1} Department of Psychology, Stanford University, CA, USA.
  
  All code and data are available at http://tinyurl.com/shapebias, the authors declare no conflict of interest, and an IRB approval was not required.
  
  https://orcid.org/0009-0009-2651-2884
...
2. The paper must include participants less than 5 years old,
3. The paper must use a word extension task that contrasts shape with other properties of the referent. 
...
All analyses were performed using the `metafor` package in R [@viechtbauer2010]. We pooled effect sizes using a multi-level meta analysis model with effect size nested within experiment number nested within paper ID as a random effect, controlling for the non-independence of effect sizes from the same experiment or paper. We attempted to perform confirmatory analysis of our preregistered hypotheses via multi-level meta-regressions including age, language, item properties of the stimuli, and syntax as fixed effects. In some cases (described below), we did not have sufficient data to add particular factors to our analyses.
...
A model incorporating language group and age as moderators revealed a significant effect size of `r round(mdlcoeff$coeff[1], 2)` ($p=$ `r papaja::apa_p(lang_model$pval)[1]`) for Indo-European speakers at the age of 36 months, with a significantly lesser effect size for the non-Indo-European group `r round(mdlcoeff$coeff[3], 2)` ($p=$ `r papaja::apa_p(lang_model$pval)[3]`). The effect experiences a gradual but significant decline with quadratic age `r mdlcoeff$coeff[4]` ($p=$ `r papaja::apa_p(lang_model$pval)[4]`) and the between-study heterogeneity remained high ($I^2 = `r round(I2_lang, 2)`$). All coefficients are shown in Table \ref{langmodel}, and Figure \ref{fig:dev-change} shows model fit compared with individual effect size estimates. 
...
Questions remain lingering about the influence of conceptual information and functionality on word meaning acquisition and the nature of hypothesis shift the child undergoes to relinquish shape bias in favor of other bases. Since functionality is closely related to complexity in the literature [@imai1997;@cimpian2005absence], more nuanced investigation of both dimensions is required to adjudicate them.

We believe this work highlights the need for more examination of the shape bias and word learning by developing consistent, reliable, and valid measures, as well as using unified procedures to facilitate clearer interpretations and theoretical comparisons.
