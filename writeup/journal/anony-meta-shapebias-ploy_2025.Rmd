...
All models including age fit better than the model without age ($\Delta_{AIC}$ between `r round(AICs$delta_aic[AICs$model == "age"], 2)` for linear and `r round(AICs$delta_aic[AICs$model == "polyage"], 2)` for polynomial). Although smaller than our pre-specified meaningful difference of 4, the polynomial model fit best ($\Delta_{AIC} =$ `r round(AICs$delta_aic[AICs$model == "polyage"],2)` ). So we adopted a polynomial function for our subsequent analysis with a significant overall estimate of `r round(polycoeff$coeff[1], 3)` ($p=$ `r papaja::apa_p(mod_poly$pval)[1]`), an age coefficient of `r round(polycoeff$coeff[2], 5)` ($p=$ `r papaja::apa_p(mod_poly$pval)[2]`) and a heterogeneity of ($I^2 = `r round(I2_poly, 2)`$).
...
In addition, the presence of substantial publication bias has profound implications for adjudicating between theoretical accounts of the shape bias. For example, theories relying on specific moderators (e.g., the effect of informative syntax or object complexity) may be untestable if the literature is skewed. Small, underpowered studies designed to test these moderators may only be published if they find a significant main effect of shape, regardless of the outcome of the moderation analysis. This practice would obscure true null findings and create a literature that makes it impossible to disentangle procedural artifacts from genuine theoretical constraints. Our inability to test for an effect of object complexity due to sparse data, despite its theoretical importance, may be a symptom of this very problem.

Broadness of the theory on what a specific response implies about the underlying knowledge and mechanisms is a double-edged sword. On the one hand, it allows for a wide range of experimental manipulations to be tested, providing a robust evidence of the phenomenon that is replicable across studies. But on the other hand, it makes it difficult to draw clear conclusions about the nature of the shape bias itself and broadly basis of label categorization and word extension. The current meta-analysis sheds light on the sparsity of the moderation space, as we found that many studies used different types of stimuli, tasks, and measures, making it challenging, even with a meta-analysis, to recognize the potentially relevant factors, and their interactions over different contexts within the same population, let alone across different populations, and arrive at a commensurable and integrative body of evidence about what a word could mean to a young child [@Almaatouq_Griffiths_Suchow_Whiting_Evans_Watts_2024]. In sum, we believe this work highlights the need for a different framework to examine the shape bias and word learning that utilizes reliable and valid measures, as well as unified procedures to facilitate clearer interpretations and theoretical comparisons. The call for a cultural shift in the field that other researchers have been making for some time now (i.e. journals prioritizing the publication of high-powered studies, encouraging the submission and acceptance of null findings through mechanisms like pre-registration/registered reports, large scale and cross-cultural/linguistic collaborative projects, emphasis on meta-science and psychometrics, ... etc), could map the road for potential mitigation of these limitation. Without these changes, we will continue 'playing 20 questions with nature'  using a possibly biased dataset, hindering true progress in understanding the origins and nature of word learning strategies.
