...
Finally, a third proposal is that children learn the shape bias from their environment, and that this learning is not necessarily mediated by language. For example, children might learn to generalize by shape because they are exposed to many objects that are similar in shape but vary in other properties such as color or texture. This exposure could be more prevalent in industrialized cultures where children are surrounded by manufactured artifacts (e.g., toys, furniture) that are often similar in shape but vary in other dimensions. In contrast, children in less industrialized cultures might be exposed to more natural objects (e.g., plants, animals) that vary more in shape and less in other dimensions. Thus, the statistical regularities of the environment could lead to a stronger shape bias in children from industrialized cultures compared with those from less industrialized cultures [@samuelson_statistical_2002; @perry2011shape; @perry2010learn; @colunga2000learning; @colunga2017not].
...
Representation across languages varied substantially, such that there were `r sum(df_shape$language == "english")` effect sizes for English while other languages included only `r sum(df_shape$language != "english")` data points. Other languages included German, Spanish, Chinese, Japanese, Korean, Vietnamese, and Tsimane. 
